...
data_analysis["order_id"] = data_analysis["order_id"].astype("object")
data_analysis["product_id"] = data_analysis["product_id"].astype("object")
data_analysis["user_id"] = data_analysis["user_id"].astype("int64")
data_analysis["age"] = data_analysis["age"].astype("int64")
print(data_analysis.head())
print(data_analysis.info())
```

![img.png](data/img/1679387956548.png)
...
![img.npg](data/img/1679388104018.png)
...
![img.npg](data/img/1679388211223.png)
...
print(data_analysis[data_analysis["event_time"] < "2000-01-01"])
data_analysis.drop(data_analysis[data_analysis["event_time"] < "2000-01-01"].index, inplace=True, axis=0)
```

![img.npg](data/img/1679388771506.png)

##    4.5 重复值处理
###       4.5.1 查看重复值
```
print(data_analysis[data_analysis.duplicated()])
```
...
![img.npg](data/img/1679388881696.png)
...
```
 - groupby()中的agg方法用处很大
 - merge()方法可以按照内连接，外连接的方法进行数据表的合并，用处很大
...
```
![img.npg](data/img/1680515689440.png)
...
![img.npg](data/img/1680516145246.png)
...
![img.npg](data/img/1680599557430.png)

![img.npg](data/img/1680600199318.png)

###     5.3.2 计算用户性别分布  基数/占比

![img.npg](data/img/1680599690318.png)

![img.npg](data/img/1680600306021.png)

###     5.3.3 计算用户地区分布  基数/占比 / 各省用户的占比

![img.npg](data/img/1680599737986.png)

![img.npg](data/img/1680600393706.png)

###     5.3.4 销售人贡献曲线(找出头部客户重点维护，二八定律-找出累计贡献销售额80%的那批用户)

![img.npg](data/img/1680599805209.png)

![img.npg](data/img/1680600534855.png)

###     5.3.5 客户消费金额的分位数

前27.21%的用户贡献了80%的销售额，共25526人,这些客户重点维护

###     5.3.6 客户总消费金额的分位数

###     5.3.7 客户总消费金额的分布

![img.npg](data/img/1680599968801.png)

![img.npg](data/img/1680600658812.png)

###     5.3.8 单笔订单消费金额的分布

![img.npg](data/img/1680600092138.png)

![img.npg](data/img/1680600739128.png)
...
###     5.4.1 用户年龄分布中  销量/销售额/

![img.npg](data/img/1681379791472.png)
###     5.4.2 用户性别分布中  销量/销售额/

![img.npg](data/img/1681379837608.png)
###     5.4.3 用户地区分布中  销量/销售额/

![img.npg](data/img/1681379883573.png)
##  5.5 时间分析
###     5.5.4 每月新客/老客人数分析  
###     5.5.5 每月新用户/老用户 销售额环比
...
![img.npg](data/img/1681379210387.png)

###     5.5.2 按日销量/销售额分布

![img.npg](data/img/1681379544548.png)

###     5.5.3 按小时分析

![img.npg](data/img/1681379599927.png)
##  5.6 RFM模型分析
...
![img.npg](data/img/1681378939105.png)
